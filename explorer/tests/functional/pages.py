...
class _BasePage:
    ...
    def __init__(self, driver: WebDriver, base_url='http://localhost:8000'):
        self._driver = driver
        self._page = None
    ...
    def read_result_rows(self):
        ...
        for row in rows[1:]:  # Drop the header row out
            results.append(
                [
                    cell.text.strip()
                    for cell in row.xpath(".//td[contains(@class, 'govuk-table__cell')]")
                ]
            )
...
class CreateQueryPage(_BasePage):
    ...
    def click_save(self):
        ...
        return QueryDetailPage(
            driver=self._driver, base_url=self._base_url, query_id=matches_expected_url.group(1)
        )
